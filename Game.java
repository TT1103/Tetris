...
{
    private static GreenfootSound backgroundMusic = new GreenfootSound("TetrisSong.mp3"); //the background music
    ...
    public void controlMovement(){
        ...
        if (curRotateDelay==0){
            if(Greenfoot.isKeyDown("up") && canRotateRight() ){
                rotateRight();
        ...
        if (Greenfoot.isKeyDown("down") && canMoveDown(y)){
            y++;
            if(!canMoveDown(y)){
                curFallDelay=20;
            }
        ...
        if( (Greenfoot.isKeyDown("shift") || Greenfoot.isKeyDown("c"))&&!shifted){
            shifted=true;
